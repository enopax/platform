...
const badgeVariants = tv({
  ...
  variants: {
    variant: {
      default: [
        "bg-brand-50 text-brand-900 ring-brand-500/30",
      ...
      outline: [
        ...
        "dark:text-gray-400 dark:ring-gray-600",
      ],
    },
  },
  defaultVariants: {
...
interface BadgeProps
  ...
  onRemove?: () => void; // Callback for the "x" button
}

const Badge = React.forwardRef<HTMLSpanElement, BadgeProps>(
  ({ className, variant, onRemove, children, ...props }: BadgeProps, forwardedRef) => {
    return (
      <span
        ref={forwardedRef}
        className={cx(badgeVariants({ variant }), className, "relative")}
      ...
      >
        ...
        {onRemove && (
          <button
            type="button"
            onClick={onRemove}
            className="ml-1 text-xs font-bold text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
