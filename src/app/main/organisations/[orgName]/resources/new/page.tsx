...
export default function AddResourcePage({ params }: AddResourcePageProps) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const { orgName: paramOrgName } = await params;
        setOrgName(paramOrgName);
        ...
        // Fetch user teams with organisation data
        const teamsResponse = await fetch('/api/team/list');
        if (teamsResponse.ok) {
          const teamsData = await teamsResponse.json();
          const teamsWithOrg = (teamsData.teams || []).map((team: any) => ({
            ...team,
            organisation: team.organisation || { name: paramOrgName }
          }));
          setUserTeams(teamsWithOrg);
        }
      } catch (error) {
  ...
  // Check if project was requested but not found
  const projectId = searchParams.get('project');
  ...
  const handleWizardComplete = (resourceData: any) => {
    ...
    if (project) {
      router.push(`/main/organisations/${orgName}/projects/${project.id}`);
    } else {
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                {project ? `Add Resource to ${project.name}` : 'Create New Resource'}
              </h1>
            </div>
            <p className="text-gray-600 dark:text-gray-300 mt-2">
              {project ? (
                ...
                useWizard
                  ? 'Follow our guided wizard to create the perfect resource for your organisation'
                  : 'Create a new resource for your organisation'
              )}
            </p>
          </div>
